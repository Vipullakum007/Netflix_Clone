...
export const getGenres = createAsyncThunk("nextflix/genres", async () => {
    const {
        data: { genres },
...
// export const fetchMovies = createAsyncThunk("nextflix/trending", async ({ type }, thunkApi) => {
...
export const getUsersLikedMovies = createAsyncThunk(
    ...
    async (email) => {
        const {
            data: { movies },
        } = await axios.get(`http://localhost:5000/api/users/liked/${email}`);
        return movies;
    }
);
...
export const removeMovieFromLiked = createAsyncThunk(
    ...
    async ({ movieId, email }) => {
        const {
            data: { movies },
        } = await axios.put("http://localhost:5000/api/users/remove", {
            email,
            movieId,
        });
        return movies;
